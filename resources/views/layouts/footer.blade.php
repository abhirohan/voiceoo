
<!-- jQuery first, then Other JS. -->
<script src="{{ route('basepath') }}/js/jquery-3.2.0.min.js"></script>
<!-- Js effects for material design. + Tooltips -->
<script src="{{ route('basepath') }}/js/material.min.js"></script>
<!-- Helper scripts (Tabs, Equal height, Scrollbar, etc) -->
<script src="{{ route('basepath') }}/js/theme-plugins.js"></script>


<!-- Select / Sorting script -->
<script src="{{ route('basepath') }}/js/selectize.min.js"></script>

<!-- Swiper / Sliders -->
<script src="{{ route('basepath') }}/js/swiper.jquery.min.js"></script>

<!-- Datepicker input field script-->
<script src="{{ route('basepath') }}/js/moment.min.js"></script>
<script src="{{ route('basepath') }}/js/daterangepicker.min.js"></script>
<!-- Init functions -->
<script src="{{ route('basepath') }}/js/main.js"></script>


<!-- Load more news AJAX script -->
<script src="{{ route('basepath') }}/js/ajax-pagination.js"></script>

<!-- Lightbox popup script-->
<script src="{{ route('basepath') }}/js/jquery.magnific-popup.min.js"></script>

<!-- Widget with events script-->
<script src="{{ route('basepath') }}/js/simplecalendar.js"></script>
<script src="{{ route('basepath') }}/js/isotope.pkgd.min.js"></script>

<script src="{{ route('basepath') }}/js/mediaelement-and-player.min.js"></script>
<script src="{{ route('basepath') }}/js/mediaelement-playlist-plugin.min.js"></script>


</body>
</html>